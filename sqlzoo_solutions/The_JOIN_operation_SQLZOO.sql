## The JOIN operation From SQLZOO
...
/*
This tutorial introduces JOIN which allows you to use data from two or more tables. The tables contain all matches and goals from UEFA EURO
...
### shorter way of saying matchid, teamid, player, gtime. Modify it to show the matchid and player name for all goals scored by Germany.
...
The FROM clause says to merge data from the goal table with that from the game table. The ON says how to figure out which rows in game go with which rows in goal the
id from goal must match matchid from game. (If we wanted to be more clear/specific we could say ON (game.id=goal.matchid)
...
FROM
    game
        JOIN
    goal ON (id = matchid AND teamid = 'GER');
...
FROM
    game
        JOIN
    goal ON (id = matchid AND player LIKE 'Mario%');
...
### 8. The example query shows all goals scored in the GermanyGreece quarterfinal. Instead show the name of all players who scored a goal against Germany.
...
SELECT 
    matchid, mdate, COUNT(player) AS 'No. of goals'
FROM
    game
        JOIN
    goal ON matchid = id
...
SELECT 
    matchid, mdate, COUNT(player) NO_of_GOALS
FROM
    game
        JOIN
    goal ON (matchid = id AND (teamid = 'GER'))
GROUP BY matchid , mdate;

-- matchid mdate NO_of_GOALS
-- 1008 9 June 2012 1
-- 1010 13 June 2012 2
-- 1012 17 June 2012 2
-- 1026 22 June 2012 4
